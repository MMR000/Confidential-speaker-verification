# -*- coding: utf-8 -*-
"""
ppsv_mpc_allinone.py — CAM++ / ASV 一体化评测（含进度条/ETA + 嵌入缓存 + dev/test 分离 +
固定点 + 后端对比 + 曲线/置信区间 + 鲁棒性 + 异盐不可链接 + MPC 压测 +（可选）PAD评测）

依赖（推荐装）：numpy tqdm matplotlib scikit-learn scipy librosa soundfile
可选：torch crypten（MPC演示用）
"""
...
def is_path_like(tok: str) -> bool:
    t = tok.strip()
    return ("/" in t or "\\" in t or t.lower().endswith(AUDIO_EXTS))
...
def save_metrics_json(report_dir: str, metrics: dict, tag: Optional[str] = None):
    ...
    fname = f"metrics{suffix}.json"
    fpath = os.path.join(report_dir or "output/report", fname)
    try:
        with open(fpath, "w", encoding="utf-8") as f:
            json.dump(metrics, f, ensure_ascii=False, indent=2)
        print(f"[SAVE] 指标摘要已保存：{fpath}")
    except Exception as e:
        print(f"[WARN] 保存 metrics.json 失败：{e}")

def safe_hash_file(path: str) -> Optional[str]:
    try:
        h = hashlib.sha256()
...
def pb_iter(it, desc=""):
    if HAS_TQDM:
        return tqdm(it, desc=desc, ncols=100)
    else:
        return it
    ...
    for root in uniq_roots:
        ...
        for dirpath, _, filenames in os.walk(root_abs):
            for fn in filenames:
                ...
                keys = {
                    fn, stem,
                    f"{parent}/{fn}", f"{parent}/{stem}",
                    rel, normpath(full)
                }
                for k in keys:
                    index.setdefault(k, full)
    print(f"[INDEX-DIRS] 文件总数={total}，映射键数={len(index)}（来自 {len(uniq_roots)} 个搜索根）")
    # 如果是 VoxCeleb 格式（wav/idxxxx/vid/00001.wav），自动构建简单 spk 列表
    spk2utts = {}
    for k, v in index.items():
        if not v.lower().endswith(AUDIO_EXTS): continue
        spk = os.path.basename(os.path.dirname(os.path.dirname(v))) if "/id" in v.replace("\\","/") else os.path.basename(os.path.dirname(v))
        spk2utts.setdefault(spk, []).append(v)
    print(f"[SPK-MAP] 从目录构建：说话人={len(spk2utts)}，录音={total}")
    return index
...
def build_audio_index_from_validated(validated_tsv: str,
    ...
    with open(validated_tsv, "r", encoding="utf-8", errors="ignore") as f:
        ...
        has_header = reader.fieldnames is not None and ("client_id" in [h.strip().lower() for h in reader.fieldnames]) \
                     and ("path" in [h.strip().lower() for h in reader.fieldnames])
        ...
        if not has_header:
            ...
            for raw in f:
                ...
                cid = parts[0].strip()
                ptk = parts[1].strip()
                abs_path = try_resolve_with_roots(ptk, roots)
                if abs_path is None and ptk in fb_index:
                    abs_path = fb_index[ptk]
                if abs_path and os.path.exists(abs_path):
                    found += 1
                    add_keys_for_path(index, cid, ptk, abs_path)
                    spk2utts.setdefault(cid, []).append(abs_path)
                    path2spk[abs_path] = cid
            print(f"[INDEX-TSV(no-header)] 行={total}，成功解析={found}，键数={len(index)}，说话人={len(spk2utts)}")
            return index, spk2utts, path2spk
        ...
        for row in reader:
            ...
            abs_path = try_resolve_with_roots(ptk, roots)
            if abs_path is None:
                for key in [ptk, os.path.splitext(ptk)[0], normpath(ptk)]:
                    if key in fb_index:
                        abs_path = fb_index[key]
                        break
            if abs_path and os.path.exists(abs_path):
                found += 1
                add_keys_for_path(index, cid, ptk, abs_path)
    ...
    print(f"[INDEX-TSV] 行={total}，成功解析={found}，键数={len(index)}，说话人={len(spk2utts)}")
    return index, spk2utts, path2spk
...
def load_trials_with_index(trials_path: str,
    ...
    with open(trials_path, "r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            total += 1
            line = raw.strip()
    ...
    print(f"[TRIALS] 总行={total}，采纳={len(pairs)}，跳过={skipped}，"
          f"{'全部带标签' if labels_ret is not None else '存在无标签或不完整标签'}")
    if not pairs:
        print("[HINT] 若 trials 写的是 client_id，请确保传入 --validated_tsv 和 --audio_root；或提供 --search_dirs。")
...
def auto_generate_trials(spk2utts: Dict[str, List[str]],
    ...
    print(f"[AUTO-TRIALS] 生成 pairs={len(pairs)}（正/负={sum(labels)}/{len(labels)-sum(labels)}），覆盖说话人={len(spk2utts)}")
    return pairs, labels
...
def pr_points(scores: np.ndarray, labels: np.ndarray, num=2048):
    thr = np.linspace(scores.min() - 1e-6, scores.max() + 1e-6, num)
    PREC, REC = [], []
    P = np.sum(labels == 1)
    for t in thr:
...
def bootstrap_ci(scores: np.ndarray, labels: np.ndarray, B=1000, seed=2024):
    ...
    n = len(scores)
    eers, aucs = [], []
    for _ in pb_iter(range(B), desc="bootstrap(pairs)") if HAS_TQDM else range(B):
...
def speaker_bootstrap_ci(pairs, labels, scores, path2spk: Optional[Dict[str,str]]=None, B=1000, seed=2024):
    ...
    rng = np.random.default_rng(seed)
    eers, aucs = [], []
    it = pb_iter(range(B), desc="bootstrap(spk)") if HAS_TQDM else range(B)
    for _ in it:
        boot_idx = []
...
def plot_and_save_curves(scores: np.ndarray, labels: np.ndarray, out_dir: str, det_probit=False):
    ...
    # DET
    try:
        eer, _, tau, FPR_arr, FNR_arr, thr_e = compute_eer_mindcf(scores, labels)
    ...
    if HAS_MPL:
        try:
            PREC, REC, _ = pr_points(scores, labels)
...
def prepare_embeddings(paths: List[str],
    ...
    """一次性提取所有路径的嵌入并应用可选变换；返回字典 path->vec。
       注意：cross-salt 评估时不要在这里加盐，而是评分阶段左右分别加。"""
    emb: Dict[str, np.ndarray] = {}

    # 先取原始嵌入（可缓存）
    it = pb_iter(paths, desc="embeddings") if HAS_TQDM else paths
    ...
    if salt_mode in {"additive", "orth", "signperm"} and salt:
        ...
        if salt_mode == "additive":
            ...
            for k, v in emb.items():
                v2 = signs * v[perm]
                emb[k] = v2 / (np.linalg.norm(v2) + 1e-12)
        print(f"[BASELINE] 已应用同盐扰动：mode={salt_mode}")
    ...
    if pca_dim and pca_dim > 0:
        ...
        try:
            ...
            for i, k in enumerate(emb.keys()):
                ...
                emb[k] = z
            print(f"[BASELINE] 已应用 PCA 降维到 {pca_dim} 维。")
        except Exception as e:
            print(f"[WARN] PCA 失败，跳过：{e}")
    ...
    if quantize in {"fp16", "int8"}:
        for k, v in emb.items():
            ...
            emb[k] = vq
        print(f"[BASELINE] 已应用量化：{quantize}")

    return emb


# -----------------------------
# 盐变换（orth / signperm / additive）
# -----------------------------
...
def score_pairs_fixed_point(pairs, emb_dict, k_bits=16, alpha_pow2=10, trunc_sigma=10):
    S = []
    it = pb_iter(pairs, desc="fixed-point") if HAS_TQDM else pairs
    for a,b in it:
...
def score_pairs_from_emb(pairs: List[Tuple[str,str]], emb: Dict[str,np.ndarray], mode="cosine",
    ...
    S = []
    it = pb_iter(pairs, desc=f"score({mode})") if HAS_TQDM else pairs
    for a, b in it:
...
def fit_lda_transform(spk2utts: Dict[str, List[str]], emb: Dict[str, np.ndarray], max_utts_per_spk=5):
    if not HAS_SK:
        ...
        return None
    X, y = [], []
    for i, (spk, utts) in enumerate(spk2utts.items()):
        cnt = 0
        for u in utts:
            if u in emb:
...
def eval_scores_and_metrics(scores: np.ndarray, pairs: List[Tuple[str,str]],
    ...
    ensure_dir(report_dir or "output/report")
    kept = len(scores)
    print(f"[EVAL] 完成 {kept} 对；分数均值={scores.mean():.6f}，std={scores.std():.6f}")
    ...
    if save_scores is not None:
        try:
            ...
            print(f"[SAVE] 评分已保存：{save_scores}")
        except Exception as e:
            print(f"[WARN] 保存评分失败：{e}")
    ...
    if labels is not None and len(labels) == kept and kept > 0:
        ...
        eer, mindcf, tau_eer, _, _, _ = compute_eer_mindcf(scores, labels_np)
        ...
        boot_pair = bootstrap_ci(scores, labels_np, B=1000, seed=2024)
        results["bootstrap_pair"] = boot_pair
        boot_spk = speaker_bootstrap_ci(pairs, labels_np, scores, path2spk=path2spk, B=1000, seed=2024)
...
def eval_trials_plaintext_fast(predictor: MVectorPredictor,
    ...
    S = []
    it = pb_iter(pairs, desc="score(cosine)") if HAS_TQDM else pairs
    for a,b in it:
        if a not in emb or b not in emb:
            S.append(0.0); continue
...
def run_baselines(predictor: MVectorPredictor,
    ...
    emb_test = prepare_embeddings(uniq_paths, predictor, pca_dim=pca_dim,
                                  ...
                                  use_cache=use_cache, cache_dir=cache_dir, raw_model_sig=raw_model_sig)
    if not emb_test:
        print("[BASELINE] 嵌入为空，基线跳过。"); return
    ...
    for m in modes:
        if m == "lda" and lda_model is None:
            print("[BASELINE] LDA 跳过（未训练成功或无 dev 数据）。"); continue
        if m == "plda" and Sigma_inv is None:
            print("[BASELINE] PLDA-like 跳过（未估计成功或无 dev 数据）。"); continue
...
def score_pairs_with_salts(pairs: List[Tuple[str, str]], emb: Dict[str, np.ndarray],
    ...
    S = []
    it = pb_iter(pairs, desc=f"salt[{salt_left}|{salt_right}]") if HAS_TQDM else pairs
    for a, b in it:
        if a not in emb or b not in emb:
            S.append(0.0); continue
...
def eval_cross_salt_unlinkability(pairs: List[Tuple[str, str]], labels: List[int],
    ...
    emb_base = prepare_embeddings(uniq_paths, predictor,
                                  ...
                                  use_cache=use_cache, cache_dir=cache_dir, raw_model_sig=raw_model_sig)
    if not emb_base:
        print("[CROSS-SALT] 嵌入为空，实验中止。"); return
...
def augment_and_write(path: str, snr_db: Optional[float], max_len_s: Optional[float], crop_mode: str="random") -> Tuple[Optional[str], Optional[str]]:
    if not HAS_AUDIO:
        return None, "未安装 soundfile/librosa，鲁棒性实验跳过。"
    try:
...
def eval_robust(predictor: MVectorPredictor,
    ...
    for snr in snr_list or []:
        new_pairs, tmp_files = [], []
        for a, b in pb_iter(pairs, desc=f"noise@{snr}dB") if HAS_TQDM else pairs:
    ...
    for L in len_list or []:
        new_pairs, tmp_files = [], []
        for a, b in pb_iter(pairs, desc=f"len@{L}s") if HAS_TQDM else pairs:
...
def eval_scale_speakers(predictor: MVectorPredictor, spk2utts: Dict[str,List[str]],
    ...
    for n in sizes:
        ...
        if len(spks) < n:
            print(f"[SCALE] 可用说话人不足 {n}，跳过。"); continue
...
def profile_mpc_matching_only(emb_pairs: List[Tuple[np.ndarray,np.ndarray]], tau: float, backend: str, iters: int):
    ...
    if backend == "local":
        for i in pb_iter(range(K), desc="mpc-local") if HAS_TQDM else range(K):
            x,y = emb_pairs[i]
            t0 = time.time()
        ...
        import torch
        for i in pb_iter(range(K), desc="mpc-crypten") if HAS_TQDM else range(K):
            x,y = emb_pairs[i]
            t0 = time.time()
...
def eval_pad(predictor_pad, pad_trials: str, index_all: Dict[str,str], report_dir: str, tag: str="_pad"):
    ...
    pairs, labels = [], []
    wavs, y = [], []
    with open(pad_trials, "r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            line = raw.strip()
            if not line or line.startswith("#"): continue
    ...
    S = []
    it = pb_iter(wavs, desc="PAD") if HAS_TQDM else wavs
    for p in it:
        try:
            s = float(predictor_pad.predict(p))  # 期望：数值越大越“真人”
        except Exception as e:
            print(f"[PAD] 失败 {p}: {e}")
            s = 0.0
        S.append(s)
...
def main():
    ...
    ap.add_argument("--dev_trials", type=str, default=None, help="development trials for backend training/calibration")
    ap.add_argument("--dev_search_dirs", nargs="*", default=None, help="dirs used ONLY for backend training")
    ...
    if args.validated_tsv:
        idx_tsv, spk2utts_test, path2spk_test = build_audio_index_from_validated(args.validated_tsv, args.audio_root, args.search_dirs)

    # 如果没有 TSV 的 speaker map，就从目录猜
    if not spk2utts_test and idx_dirs:
        tmp = {}
        for k, v in idx_dirs.items():
            if not v.lower().endswith(AUDIO_EXTS): continue
            spk = os.path.basename(os.path.dirname(os.path.dirname(v))) if "/id" in v.replace("\\","/") else os.path.basename(os.path.dirname(v))
            tmp.setdefault(spk, []).append(v)
        spk2utts_test = tmp
        path2spk_test = {p: s for s, us in tmp.items() for p in us}

    index_all = dict(idx_dirs); index_all.update(idx_tsv)
    print(f"[INDEX-MERGED(test)] 索引键总数={len(index_all)}（TSV 优先级高于目录）")
    ...
    if args.dev_validated_tsv:
        dev_index_tsv, dev_spk2utts, dev_path2spk = build_audio_index_from_validated(args.dev_validated_tsv, args.audio_root, args.dev_search_dirs)
    else:
        dev_index_tsv = {}
    dev_index_all = dict(dev_index_dirs); dev_index_all.update(dev_index_tsv)
    print(f"[INDEX-MERGED(dev)] 索引键总数={len(dev_index_all)}")
    ...
    if args.auto_trials:
        if not spk2utts_test:
            print("[ERROR] --auto_trials 需要能从目录或 TSV 解析出说话人映射（检查 --search_dirs 或 --validated_tsv）。")
            sys.exit(1)
        ...
        if not pairs:
            print("[ERROR] trials 解析为空，请检查。"); sys.exit(1)
    else:
        print("[INFO] 未提供 --trials 且未 --auto_trials，批量评测将跳过（仍可做单次匹配）。")

    # 3b) dev pairs+labels（仅训练/标定）
    ...
    if args.dev_trials:
        ...
        if not dev_pairs:
            print("[WARN] dev_trials 解析为空，将无法进行无泄漏后端训练与阈值标定。")

    # 4) 评测：CAM++ 明文分数（test）
    if pairs:
        print(f"[EVAL] 载入 {len(pairs)} 对样本（labels: {'yes' if labels is not None else 'no'}）")
        ...
        if args.auto_tau and res_float.get("tau_eer") is not None:
            ...
            print(f"[EVAL] 使用 test 集浮点 EER 阈值：{tau_for_demo:.4f}")

        # 4b) 固定点：dev 标定 -> test 评估
        ...
        if dev_pairs and dev_labels:
            ...
            eerd, _, tau_eer_dev, _, _, _ = compute_eer_mindcf(S_dev, dev_labels_np)
            ...
            print("[FIXED-POINT] 未提供 dev 数据：固定点阈值退化为 --threshold 或 test 浮点 EER 阈值。")
        ...
        if ("lda" in args.baselines or "plda" in args.baselines):
            if dev_pairs:
                ...
                print("[BASELINE] 无 dev 数据，后端训练将跳过。")
        if args.baselines:
        ...
        if (args.robust_noise or args.robust_len):
            if labels is None:
                print("[ROBUST] 缺少标签，鲁棒性 EER 评估跳过。")
            else:
        ...
        if args.profile_embed > 0:
            ...
            t = []
            for i in pb_iter(range(K), desc="embed-time") if HAS_TQDM else range(K):
    ...
    if args.audio1 and args.audio2:
        ...
        if not a1 or not os.path.exists(a1):
            print(f"[ERROR] audio1 无法解析为有效文件：{args.audio1}"); sys.exit(2)
        if not a2 or not os.path.exists(a2):
            print(f"[ERROR] audio2 无法解析为有效文件：{args.audio2}"); sys.exit(2)

        print(f"[MPC] 后端：{args.backend}，阈值：{tau_for_demo:.4f}")
        if args.backend == "local":
            score, same = mpc_compare_local(predictor, a1, a2, tau_for_demo)
        else:
            score, same = mpc_compare_crypten(predictor, a1, a2, tau_for_demo)
        print(f"[MPC-{args.backend}] 相似度：{score:.6f} -> 判定：{'同一人' if same else '不同人'}（阈值 {tau_for_demo:.4f}）")
