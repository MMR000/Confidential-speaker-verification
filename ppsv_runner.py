...
def compute_eer_mindcf(scores: np.ndarray, labels: np.ndarray,
    ...
    # 准备阈值：唯一值；若过多则做等间距栅格（最多 4096 个）
    thr = np.unique(scores)
    MAX_THR = 4096
    if thr.shape[0] > MAX_THR:
        thr = np.linspace(thr.min() - 1e-6, thr.max() + 1e-6, MAX_THR).astype(np.float64)
    else:
    ...
    if P == 0 or N == 0:
        raise ValueError("标签全为同类，无法计算 EER/minDCF")
    ...
    FPR_arr = np.asarray(FPR_list, dtype=np.float64)
    FNR_arr = np.asarray(FNR_list, dtype=np.float64)
    ...
    if len(idx) == 0:
        ...
        k = int(idx[0])
        x0, x1 = float(thr[k]), float(thr[k + 1])
        y0, y1 = float(diff[k]), float(diff[k + 1])
    ...
    return float(eer), float(min_dcf), float(tau), FPR_arr, FNR_arr, thr
...
def fit_lda_transform(spk2utts: Dict[str, List[str]], emb: Dict[str, np.ndarray], max_utts_per_spk=5):
    ...
    # 将 items 固定成 list，再交给进度条；并在循环里自己 enumerate
    items = list(spk2utts.items())
    it = pb_iter(items, desc="LDA收集", total=len(items)) if HAS_TQDM else items
    ...
    for spk, utts in it:
        ...
        for u in utts or []:
            if u in emb:
                X.append(emb[u]); y.append(cls_id); cnt += 1
    ...
    try:
        lda.fit(X, y)
        print(f"[BASELINE] LDA 训练完成（类数={len(set(y))}，样本数={len(y)}，每类最多 {max_utts_per_spk} 条）。")
        return lda
    except Exception as e:
...
def eval_scores_and_metrics(scores: np.ndarray, pairs: List[Tuple[str,str]],
    ...
    if labels is not None and len(labels) == kept and kept > 0:
        ...
        boot_pair = bootstrap_ci(scores, labels_np, B=20, seed=2024)
        results["bootstrap_pair"] = boot_pair
        boot_spk = speaker_bootstrap_ci(pairs, labels_np, scores, path2spk=path2spk, B=20, seed=2024)
...
def main():
    ...
    with StageTimer("索引(dev)"):
        ...
        # 新增：若 dev_spk2utts 为空但 dev_index_dirs 存在，则从目录聚合说话人，供 LDA 使用
        if not dev_spk2utts and dev_index_dirs:
            tmp = {}
            it_src = dev_index_dirs.items()
            for k, v in pb_iter(it_src, desc="聚合说话人(dev)", total=len(dev_index_dirs)) if HAS_TQDM else it_src:
                pth = v
                if not pth.lower().endswith(AUDIO_EXTS):
                    continue
                spk = os.path.basename(os.path.dirname(os.path.dirname(pth))) if "/id" in pth.replace("\\","/") \
                      else os.path.basename(os.path.dirname(pth))
                tmp.setdefault(spk, []).append(pth)
            dev_spk2utts = tmp
            dev_path2spk = {pp: ss for ss, us in tmp.items() for pp in us}
            print(f"[INDEX-AGG(dev)] 由目录聚合：说话人={len(dev_spk2utts)}")
